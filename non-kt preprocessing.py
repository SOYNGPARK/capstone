...
# 상품명 1차 전처리 리스트
re_dic = ['\s?\([a-zA-Z!"#$%&\'*+,-./:;<=>?@\\^_`|~＆Ⅱ\s]+[\d]*[a-zA-Z!"#$%&\'*+,-./:;<=>?@\\^_`|~＆Ⅱ\s]+[\d]*\)\s?',  # (알파벳은 무조건 나머지는 있어도 되고 없어도 됨) ex.(Bridget Jones`s Baby)
          '\s?\([가-힣a-zA-Z\s!"#$%&\'*+,-./:;<=>?@\\^_`|~＆]+\)\s?', # (숫자빼고다) ex.(극장판), 
          ...
          '\s?극장판\s?', '\s?소장용\s?', '\s?Classic\s?', '\s클래식$', 
          '\s?예약구매\s?', '\s?3D\s?'] 
...
# 상품명 2차 전처리 리스트
re_dic_after = ['\s?\(본편', '\s?\(제', '\s?\(\d*[가-힣\s]+\d*\)\s?', '\s?\[\s?', '\s?\]\s?']
